...
public class PessoaDAO {
	...
	try{
		...
		stmt = con.prepareStatement("INSERT INTO Pessoa (nome, cpf, dataNascimento, telefone, rua, numero,bairro, complemento, cep,cidade,estado)VALUES (?,?,?,?,?,?,?,?,?,?,?) ");
	...
	try{
		...
		stmt = con.prepareStatement("UPDATE Pessoa SET nome = ?, cpf = ?, dataNascimento = ?, telefone = ?, rua = ?, numero = ?,bairro = ?, complemento = ?, cep = ?,cidade = ?,estado = ? WHERE id = ?");
		stmt.setString(1,p.getNome());
		stmt.setString(2,p.getCpf());
		stmt.setString(3,p.getDataNascimento());
		stmt.setString(4,p.getTelefone());
